...
class CVBertOutput(ModelOutput):
    ...
    prior_logvar: torch.FloatTensor = None
...
class CVBertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.y_predictor_net_fc2 = nn.Linear(config.y_inter_dim, config.y_output_dim)
...
class CVBertForTraining(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None and user_group_labels is not None:
            ...
            print(user_group_scores)
            print(user_group_labels)
            print(user_group_scores.shape)
            print(user_group_labels.shape)
